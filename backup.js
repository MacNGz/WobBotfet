...
function deleteGame(game){
    ...
    delete GAMES[p2].games[p1];
    GAMES[p1].numGames -= 1;
    GAMES[p2].numGames -= 1;

    console.log(GAMES[p1].numGames);
    console.log(GAMES[p2].numGames);
}
    ...
    if (message.content.substring(0, 2) === 'c!') {
        ...
        switch(cmd) {
            case 'connect4':
                ...
                message.channel.send(mention(p2) + " " + message.author.username + " has challenged you to Connect 4.").catch(console.error);
                message.channel.send(boardString).catch(console.error);
                break;
